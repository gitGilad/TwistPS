...
long get_total_cpu_time() {
	...
	while(fgets(line, 300, statf)) {
		if(strncmp(line, "cpu", 3) == 0) {
			sscanf(line, "%*s %lu %lu %lu %lu", &user, &nice, &system, &idle);
		}
...
void get_cpu_times(std::map<long, int> & cpu_before_times) {
	...
	while((ent = readdir(proc))) {
	    ...
	    cpu_before_times.insert(std::pair<long, int>(pid, get_process_cpu_time(pid)));
	}

	cpu_before_times.insert(std::pair<long, int>(-1, get_total_cpu_time()));
...
double calculate_cpu(std::map<long, int> cpu_before_times, long pid) {
	std::map<long, int>::iterator it = cpu_before_times.find(pid);

	if (it == cpu_before_times.end()) {
		return -1;
	}

	long ptime = get_process_cpu_time(pid) - it->second;
...
void print_status(long pid, std::map<long, int> cpu_before_times) {
...
void iterate_proc_and_print_info(std::map<long, int> before_times) {
...
{
	std::map<long, int> cpu_before_times;
